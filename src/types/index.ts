...
export function mdxNodeIntoPost(
  mdxNode: MdxNode,
  bodyComponent: ReactNode | null | undefined
): Post {
  ...
  if (!mdxNode.internal?.contentFilePath) {
    throw Error(
      `Post missing file path. Post slug: ${mdxNode.fields.slug}. Aborting.`
    );
  }
...
export const queryIntoPost = (
  ...
  return mdxNodeIntoPost(postData, bodyComponent);
};
...
export const queryIntoListing = (listing: MdxListingQuery): Array<Post> => {
  ...
  return nodes.map((node) => mdxNodeIntoPost(node, null));
};
